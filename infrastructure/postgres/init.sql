...
-- ─── Companion databases (FIX audit P0-12) ───────────────────────
-- keycloak (KC_DB_URL=.../keycloak) and mlflow (MLFLOW_BACKEND_STORE_URI=
-- .../mlflow) connect to their OWN databases. Nothing created them before,
-- so both containers crash-looped on first boot.
...
-- NOTE (FIX audit P0-2): `pgaudit` was removed — the stock postgres:16
-- image does not ship that library, so `shared_preload_libraries=pgaudit`
-- made the server FATAL on boot (crash loop taking the whole stack down).
-- For DB-level audit logging use a pgaudit-enabled image before re-adding it.
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
-- PROD-RUNTIME FIX: pgvector is intentionally not enabled here. The stack uses Qdrant,
-- and the stock postgres:16 image does not ship the `vector` extension.
