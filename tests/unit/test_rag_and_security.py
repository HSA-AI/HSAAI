...
class TestABACClient:
    ...
    def test_abac_fails_closed_on_opa_unreachable(self, monkeypatch):
        import sys, asyncio
        sys.path.insert(0, "/app/packages")
        monkeypatch.setenv("ABAC_ENABLED", "true")
        monkeypatch.setenv("OPA_URL", "http://nonexistent:9999")
        monkeypatch.setenv("OPA_TIMEOUT", "0.5")
        monkeypatch.delenv("ABAC_FAIL_OPEN", raising=False)
        import importlib
        import common.abac
        importlib.reload(common.abac)
        from common.abac import check_access
        # check_access is async — must run via event loop.
        # FIXED (audit): this test previously asserted fail-OPEN on OPA
        # unreachability — the exact insecure behavior removed by
        # "FIX v0.1 (P0)" in common/abac.py (confidential documents were
        # readable during OPA outages). The secure default is fail-CLOSED;
        # fail-open requires an explicit ABAC_FAIL_OPEN=true (dev only).
        result = asyncio.run(check_access(
            user={"sub": "u1", "roles": ["ai_user"], "tenant_id": "default"},
            action="documents:read",
            resource={"type": "document", "tenant_id": "default"},
        ))
        assert result is False  # fail-closed (secure default)

    def test_abac_fails_open_only_when_explicitly_enabled(self, monkeypatch):
        import sys, asyncio
        sys.path.insert(0, "/app/packages")
        monkeypatch.setenv("ABAC_ENABLED", "true")
        monkeypatch.setenv("OPA_URL", "http://nonexistent:9999")
        monkeypatch.setenv("OPA_TIMEOUT", "0.5")
        monkeypatch.setenv("ABAC_FAIL_OPEN", "true")  # explicit dev escape hatch
        import importlib
        import common.abac
        importlib.reload(common.abac)
        from common.abac import check_access
        ...
        assert result is True  # documented opt-in behavior
