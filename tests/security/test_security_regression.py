...
class TestDockerComposeSecurity:
    ...
    def test_elasticsearch_security_enabled(self):
        for filename in ["docker-compose.hsa-internal.yml", "docker-compose.production.yml"]:
            path = PROJECT_ROOT / filename
            if not path.exists():
                continue
            for name, service in yaml.safe_load(path.read_text()).get("services", {}).items():
                if "elasticsearch" in name or "elasticsearch" in service.get("image", ""):
                    env = service.get("environment", {})
                    if isinstance(env, list): env = dict(x.split("=", 1) for x in env if "=" in x)
                    assert str(env.get("xpack.security.enabled", "")).lower() == "true"
