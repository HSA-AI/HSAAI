...
class VaultClient:
    ...
    def get_secret_value_sync(self, path: str, key: str, default: str = None) -> str:
        ...
        FIX v0.1 (P0): Previously the error-handling was broken:
          1. `loop.is_running()` returns True in async context
          2. `raise RuntimeError(...)` is raised
          3. `except RuntimeError: pass` catches it
          4. `asyncio.run(...)` is called anyway → raises another RuntimeError
          5. The function never returns
        Now: properly detects async context and uses
        `asyncio.run_coroutine_threadsafe` with a dedicated background loop
        if called from async context, or `asyncio.run` for sync contexts.
        """
        import asyncio
        try:
            loop = asyncio.get_running_loop()
            # We're inside an async context — can't use asyncio.run()
            # Schedule the coroutine on a dedicated background thread+loop
            import threading
            result_holder: list = []
            error_holder: list = []
            def _run_in_bg_loop():
                ...
                try:
                    result_holder.append(bg_loop.run_until_complete(
                        self.get_secret_value(path, key, default)
                    ))
                except Exception as e:
                    error_holder.append(e)
                finally:
                    bg_loop.close()
            t = threading.Thread(target=_run_in_bg_loop, daemon=True)
            t.start()
            t.join(timeout=30)
            if error_holder:
                raise error_holder[0]
            if not result_holder:
                raise RuntimeError("Background secret fetch timed out")
