...
Entities (JSON list):"""
        ...
        graph_entities = []
        try:
            # FIX v0.1 (P0): Build parameterized Cypher query and pass
            # parameters separately to prevent Cypher injection.
            cypher = self._build_graph_query(entities, tenant_id, top_k)
            resp = await self.client.post(
                f"{self.rag_url}/v1/graph/query",
                json={
                    "cypher": cypher,
                    "params": {  # FIX v0.1 (P0): pass parameters separately
                        "tenant": tenant_id,
                        "names": entities[:5],
                        "limit": min(top_k * 2, 200),
                    },
                    "tenant_id": tenant_id,
                    "limit": min(top_k * 2, 200),
                },
    ...
    def _build_graph_query(self, entities: List[str], tenant_id: str, limit: int) -> str:
        ...
        SECURITY FIX v0.1 (P0): Previously interpolated `tenant_id` and
        `entities` directly into Cypher, allowing injection (e.g.,
        `tenant_id = "x' OR 1=1 //"`). Now uses parameterized Cypher with
        `$tenant` and `$names` parameters. Caller must pass these to
        `session.run(query, tenant=..., names=...)`.
        """
        # FIX v0.1 (P0): Clamp limit to a safe range to prevent DoS
        safe_limit = max(1, min(int(limit), 100))
        if not entities:
            return (
                "MATCH (n:Entity) WHERE n.tenant_id = $tenant "
