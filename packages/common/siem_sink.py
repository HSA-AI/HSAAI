...
async def _send_to_siem(entries: list[dict]) -> None:
    ...
    FIX v0.1 (P0): Previously called async `_send_to_splunk` without `await`,
    so the coroutine was never scheduled and no logs reached Splunk. Now
    properly awaits all async backend functions."""
    if SIEM_BACKEND == "splunk":
        await _send_to_splunk(entries)  # FIX v0.1 (P0): add await
    elif SIEM_BACKEND == "sentinel":
        await _send_to_sentinel(entries)  # FIX v0.1 (P0): add await
    elif SIEM_BACKEND == "cloudwatch":
        await _send_to_cloudwatch(entries)  # FIX v0.1 (P0): add await
    elif SIEM_BACKEND == "webhook":
        await _send_to_webhook(entries)  # FIX v0.1 (P0): add await
    else:
