    ...
    try:
        async with httpx.AsyncClient(timeout=OPA_TIMEOUT) as client:
            ...
            if response.status_code >= 400:
                ...
                # FIX v0.1 (P0): Was failing OPEN for read actions during OPA
                # outages — allowed access to confidential documents by
                # unauthorized users during network blips. Now fails CLOSED
                # for ALL actions unless ABAC_FAIL_OPEN=true (dev only).
                if os.getenv("ABAC_FAIL_OPEN", "false").lower() == "true":
                    logger.warning("OPA returned HTTP %d for action '%s' — failing OPEN (ABAC_FAIL_OPEN=true)", response.status_code, action)
                    return True
                logger.error("OPA returned HTTP %d for action '%s' — failing CLOSED (deny)", response.status_code, action)
                return False
            result = response.json()
            allowed = bool(result.get("result", False))
    except httpx.HTTPError as exc:
        # FIX v0.1 (P0): Same fail-closed for network errors.
        if os.getenv("ABAC_FAIL_OPEN", "false").lower() == "true":
            logger.warning("OPA unreachable for action '%s': %s — failing OPEN (ABAC_FAIL_OPEN=true)", action, exc)
            return True
        logger.error("OPA unreachable for action '%s': %s — failing CLOSED (deny)", action, exc)
        return False
