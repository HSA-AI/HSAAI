...
def _get_jwks_client() -> Optional[PyJWKClient]:
    ...
    if _jwks_client is None:
        try:
            _jwks_client = PyJWKClient(os.getenv("KEYCLOAK_JWKS_URL") or f"{KEYCLOAK_ISSUER}/protocol/openid-connect/certs", timeout=10)
...
def _extract_roles(claims: dict) -> list[str]:
    ...
    if isinstance(resource_access, dict):
        allowed_clients = {x.strip() for x in os.getenv("HSAAI_ROLE_CLIENT_IDS", KEYCLOAK_AUDIENCE).split(",") if x.strip()}
