...
class SemaphoreBulkhead:
    ...
    def __init__(self, name: str, max_concurrent: int = 10, max_queue: int = 5, timeout: float = 30.0):
        ...
        # FIX v0.1 (P0): Use asyncio.Semaphore instead of threading.Semaphore.
        # threading.Semaphore.acquire() is blocking — freezes the event loop.
    ...
    async def __aenter__(self):
        # FIX v0.1 (P0): Use async acquire with timeout
        try:
            await asyncio.wait_for(self._queue_semaphore.acquire(), timeout=self.timeout)
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        # FIX v0.1 (P0): Only release _queue_semaphore (the one we acquired).
        # Previously released _semaphore too — was never acquired, causing
        # "Semaphore released too many times" ValueError after max_concurrent+1 exits.
        async with self._lock:
