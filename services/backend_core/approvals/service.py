...
async def create_approval(
    ...
    *,
    ...
    requires_two_person = requires_two_person or risk_level == "critical"
    ...
    *,
    ...
    if requires_two:
        ...
        if row.status == STATE_PENDING_FIRST_APPROVAL:
            ...
            _dispatch_notification(row, stage="second")
            return row
        # Second approval
        if row.status == STATE_PENDING_SECOND_APPROVAL:
            if approver in {row.approver, row.requester}:
...
# Cancel / Expire
# ═══════════════════════════════════════════════════════════════════
def cancel_approval(db: Session, *, request_id: str, actor: str, reason: str = "", allow_admin: bool = False) -> ApprovalRequest:
    row = db.query(ApprovalRequest).filter(ApprovalRequest.request_id == request_id).first()
    if row is None:
        raise ValueError("Approval request not found")
    if actor != row.requester and not allow_admin:
        raise ValueError("Only requester or authorized admin can cancel")
...
# SLA + escalation
# ═══════════════════════════════════════════════════════════════════
def check_sla_breaches(db: Session, *, tenant_id: str | None = None, workspace_id: str | None = None) -> List[ApprovalRequest]:
    ...
    """
    # FIX v0.1 (P0): Bug — `if True` always selected two-person states,
    # never detecting SLA breaches for single-approver requests (STATE_PENDING).
    # Now includes ALL pending states so breaches are detected for both
    # single-approver and two-person-rule workflows.
    pending_states = [
        STATE_PENDING,
    ...
    now = _now()
    query = db.query(ApprovalRequest).filter(ApprovalRequest.status.in_(pending_states))
    ...
    for row in rows:
        ...
        if esc_count >= MAX_ESCALATIONS:
            ...
            _dispatch_notification(row, stage="escalation")
...
def list_approvals(
    ...
    if workspace_id is not None:
        q = q.filter(ApprovalRequest.workspace_id == workspace_id)
    if status:
...
def _dispatch_notification(row, stage):
    # Retain task references until delivery finishes. Sync callers have no loop.
    try:
        loop = asyncio.get_running_loop()
    except RuntimeError:
        asyncio.run(notify_approval(row, stage=stage))
    else:
        task = loop.create_task(notify_approval(row, stage=stage))
        _notification_tasks.add(task)
        task.add_done_callback(_notification_tasks.discard)


async def notify_approval(row: ApprovalRequest, stage: str = "first") -> None:
    """Best-effort async notification. Never blocks the workflow.

    FIX v0.1 (P0): Previously used sync `smtplib.SMTP` + sync `httpx.post`
    inside async `create_approval`, blocking the event loop for up to 30s
    (3 webhooks × 10s timeout + SMTP). Now fully async:
      - aiosmtplib for SMTP (if available, falls back to thread executor)
      - httpx.AsyncClient for webhooks
    ...
    prefix = {
        ...
        "second": "HSAAI approval: second approver needed (two-person rule)",
    ...
    async def _send_email():
        if not (settings.smtp_host and settings.approval_email_from):
            return
        try:
            # Try aiosmtplib first (true async)
            try:
                import aiosmtplib
                email = EmailMessage()
                ...
                # Fallback: run sync smtplib in thread executor (non-blocking)
                await asyncio.get_event_loop().run_in_executor(None, _send_email_sync, message, prefix)
        except Exception as e:
            logger.warning("notify_approval: email send failed: %s", type(e).__name__)

    def _send_email_sync(message: str, prefix: str):
        """Sync SMTP send — called from thread executor."""
        email = EmailMessage()
        email["From"] = settings.approval_email_from
        email["To"] = settings.approval_email_from
        email["Subject"] = prefix
    ...
    async def _send_webhook(url: str):
        try:
            ...
            logger.warning("notify_approval: webhook failed: %s", type(e).__name__)

    # FIX v0.1 (P0): Send all notifications in parallel (asyncio.gather)
    webhook_urls = [u for u in [settings.teams_webhook_url, settings.slack_webhook_url, settings.approval_webhook_url] if u]
    tasks = [_send_email()] + [_send_webhook(u) for u in webhook_urls]
