...
AGENT_PREFIX = {
    ...
    "general": "HSAAI Assistant",
}

def _search_rag_engine(message: str, tenant_id: str, workspace_id: str) -> list[dict[str, Any]]:
    """FIX (P0 runtime): was `async def` but process_message calls it
    synchronously — the returned coroutine hit `_build_context`'s for-loop and
    raised `TypeError: 'coroutine' object is not iterable` → /v1/chat 500.
    Converted to a sync httpx call; returns [] when the RAG engine is
    unreachable (dockerless demo) exactly as before."""
    try:
...
def _call_orchestrator(message: str, agent: str, tenant_id: str, workspace_id: str, context: str, task: str | None = None, system_prompt: str | None = None, knowledge_scopes: list[str] | None = None) -> tuple[str, dict[str, Any]]:
    """FIX (P0 runtime): was `async def` but process_message consumes its
    result synchronously (`response, orchestration = _call_orchestrator(...)`)
    — the coroutine could not be unpacked → /v1/chat 500. Converted to a sync
    httpx call; the graceful ("", {error}) fallback when the orchestrator is
    unreachable (dockerless demo) is preserved verbatim."""
...
def process_message(user: str, message: str, workspace_id: str = "default", tenant_id: str = "default", claims: dict[str, Any] | None = None) -> dict:
    ...
    agent_name = resolved.name if resolved.key != "general" else AGENT_PREFIX.get(agent, "HSAAI Assistant")

    save_message(user, "user", message, workspace_id, agent, tenant_id=tenant_id)
    ...
    # FIX (P0 runtime): detect_intent returns a plain dict — `.intent` attribute
    # access raised AttributeError. Use dict access with fallbacks.
    response, orchestration = _call_orchestrator(message, agent, tenant_id, workspace_id, context, task=intent_result.get("intent") if isinstance(intent_result, dict) else getattr(intent_result, "intent", None), system_prompt=resolved.system_prompt, knowledge_scopes=resolved.knowledge_scopes)
    if not response:
    ...
    save_message(user, "assistant", response, workspace_id, agent, tenant_id=tenant_id)
    ...
    return {
        ...
        "intent": intent_result.get("intent") if isinstance(intent_result, dict) else getattr(intent_result, "intent", "general_query"),
        "intent_score": intent_result.get("confidence") if isinstance(intent_result, dict) else getattr(intent_result, "score", 0.0),
