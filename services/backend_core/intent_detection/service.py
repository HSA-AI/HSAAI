...
def _llm_classify(query: str) -> dict[str, Any] | None:
    ...
    FIX (P0 runtime): this function was `async` but its only caller
    (detect_intent, line ~98) invokes it synchronously without await, so
    llm_intent was a coroutine and `llm_intent.get(...)` raised
    `AttributeError: 'coroutine' object has no attribute 'get'` → the whole
    /v1/chat pipeline returned 500. Converted to a sync httpx call to match
    the (synchronous) call chain — behaviour unchanged when the LLM gateway
    is unreachable (returns None, keyword fallback runs).
    """
    try:
