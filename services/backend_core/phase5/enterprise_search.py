...
async def _search_rag(query: str, tenant_id: str, workspace_id: str, top_k: int = 5) -> list[dict]:
    ...
    try:
        ...
        raise RuntimeError("RAG dependency unavailable") from exc
...
def unified_search(req: EnterpriseSearchRequest) -> dict:
    ...
    try:
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
                rag_results = ex.submit(context.run, lambda: asyncio.run(_search_rag(req.query, req.context.tenant_id, req.context.workspace_id, req.top_k))).result()
        except RuntimeError:
        ...
        rag_results = []
        rag_error = "RAG dependency unavailable"
    ...
    return {
        ...
        "answer": answer,
        "elapsed_ms": elapsed,
        "real_search_performed": rag_error is None,
        "status": "completed" if rag_error is None else "failed",
        "error": rag_error,
    }
