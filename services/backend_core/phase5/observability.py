...
def read_events(limit: int = 200, *, tenant_id: str | None = None, workspace_id: str | None = None) -> list[dict]:
    _ensure_store()
    ...
    out = [e for e in out if (tenant_id is None or e.get("tenant_id") == tenant_id) and (workspace_id is None or e.get("workspace_id") == workspace_id)]
...
def ai_metrics(*, tenant_id: str | None = None, workspace_id: str | None = None) -> dict:
    """
    Compute real AI metrics from operational event data.
    All metrics are calculated from actual events — no placeholders.
    """
    events = read_events(500, tenant_id=tenant_id, workspace_id=workspace_id)
    total = len(events)
