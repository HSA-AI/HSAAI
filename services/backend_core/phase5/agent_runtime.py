...
import httpx
from common.security.request_policy import outgoing_headers
...
def list_agents() -> dict:
    return {"count": len(AGENTS), "agents": [{"id": k, **v, "status": "configured"} for k, v in AGENTS.items()]}
...
async def _call_llm(prompt: str, system: str, model: str, temperature: float = 0.2, max_tokens: int = 1024) -> tuple[str, str | None]:
    ...
    try:
        ...
        return "", f"LLM gateway returned HTTP {response.status_code}"
...
def run_agent(req: AgentRunRequest) -> dict:
    ...
    try:
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
                answer, llm_error = ex.submit(context.run, lambda: asyncio.run(_call_llm(req.task, system_prompt, model_route["model"]))).result()
        except RuntimeError:
    ...
    if not answer and not llm_error:
        llm_error = "Model returned an empty response"
    succeeded = bool(answer) and llm_error is None
