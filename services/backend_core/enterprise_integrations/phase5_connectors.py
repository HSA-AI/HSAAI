...
def _connector_failure(exc: Exception) -> Dict[str, Any]:
    """Never return or log provider bodies, URLs, credentials or exception text."""
    logger.warning("Connector request failed (%s)", type(exc).__name__)
    return {"error": "Connector request failed", "error_type": type(exc).__name__}
...
async def retry_with_backoff(
    ...
    if max_retries < 0 or not all(math.isfinite(value) and value >= 0 for value in (base_delay, max_delay)):
        raise ValueError("Retry count and finite delays must be non-negative")
        ...
        try:
            ...
            if retry_after:
                ...
                if math.isfinite(requested):
                    delay = min(max_delay, max(0.0, requested))
            ...
            if attempt == max_retries:
                raise
            delay = min(base_delay * (2 ** attempt), max_delay)
            logger.warning("Retry %d/%d after %.2fs (%s)", attempt + 1, max_retries, delay, type(e).__name__)
            await asyncio.sleep(delay)
...
class Dynamics365Connector:
    ...
    async def _get_token(self) -> str:
        cache_key = f"dynamics:{self.tenant_id}:{self.client_id}:{self.base_url.rstrip('/')}"
