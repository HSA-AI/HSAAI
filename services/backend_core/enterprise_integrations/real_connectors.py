...
async def _get_ms_graph_token(tenant_id: str, client_id: str, client_secret: str, scope: str = "https://graph.microsoft.com/.default") -> str:
    """Get MS Graph OAuth2 token using client credentials flow."""
    cache_key = f"msgraph:{tenant_id}:{client_id}:{scope}"
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        if resp.status_code >= 400:
            raise RuntimeError(f"MS Graph auth failed: {resp.status_code}")
...
class SharePointConnector:
    ...
    async def list_files(self, folder_path: str = "/") -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if resp.status_code >= 400:
                logger.error("SharePoint list_files failed: HTTP %d", resp.status_code)
                return {"error": f"Upstream HTTP {resp.status_code}", "files": []}
            data = resp.json()
            files = [
                {
                    "id": item["id"],
    ...
    async def download_file(self, file_id: str) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "content": None}
    ...
    async def search_files(self, query: str) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "results": []}
            data = resp.json()
            results = [
...
class PowerBIConnector:
    ...
    async def list_dashboards(self) -> dict[str, Any]:
        ...
        token = await _get_ms_graph_token(self.tenant_id, self.client_id, self.client_secret, scope="https://analysis.windows.net/powerbi/api/.default")
        headers = {"Authorization": f"Bearer {token}"}
        url = f"{self.base_url}/groups/{self.workspace_id}/dashboards" if self.workspace_id else f"{self.base_url}/dashboards"
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "dashboards": []}
            data = resp.json()
            dashboards = [
    ...
    async def list_reports(self) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "reports": []}
            data = resp.json()
            reports = [
    ...
    async def list_datasets(self) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "datasets": []}
            data = resp.json()
            datasets = [
...
class OutlookConnector:
    ...
    def __init__(self):
        ...
        self.sender_email = os.getenv("OUTLOOK_SENDER_UPN", "")
        self.base_url = "https://graph.microsoft.com/v1.0"
    ...
    async def send_mail(self, to: list[str], subject: str, body: str, html: bool = False) -> dict[str, Any]:
        ...
        token = await _get_ms_graph_token(self.tenant_id, self.client_id, self.client_secret)
        headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
        # FIX v0.1 (P0): /users/me requires delegated permission flow, but
        # _get_ms_graph_token uses client_credentials (app-only). The correct
        # URL for app-only flow is /users/{user_id_or_UPN}/sendMail.
        # Use sender email as the user identifier.
        sender_user = self.sender_email or os.getenv("OUTLOOK_SENDER_UPN")
        if not sender_user:
            return {"error": "OUTLOOK_SENDER_UPN env var required for app-only flow", "sent": False}
        url = f"{self.base_url}/users/{sender_user}/sendMail"
        payload = {
            "message": {
                "subject": subject,
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "sent": False}
    ...
    async def list_calendar_events(self, user_email: str = "me", top: int = 10) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "events": []}
            data = resp.json()
            events = [
                {
                    "id": e["id"],
...
class ITSMConnector:
    ...
    async def create_ticket(self, title: str, description: str, priority: str = "medium", category: str = "incident") -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(url, headers=headers, json=payload)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "ticket_id": None}
            data = resp.json()
    ...
    async def get_ticket(self, ticket_id: str) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "ticket": None}
    ...
    async def list_tickets(self, status: str = "open", limit: int = 50) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "tickets": []}
            data = resp.json()
...
class DMSConnector:
    ...
    async def get_document(self, doc_id: str) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "document": None}
    ...
    async def get_document_versions(self, doc_id: str) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "versions": []}
            data = resp.json()
...
class SuccessFactorsConnector:
    ...
    async def _get_token(self) -> str:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(url, data=data)
            if resp.status_code >= 400:
                raise RuntimeError(f"SuccessFactors auth failed: HTTP {resp.status_code}")
            token_data = resp.json()
    ...
    async def get_employee(self, employee_id: str) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "employee": None}
            data = resp.json()
    ...
    async def list_employees(self, top: int = 50) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "employees": []}
            data = resp.json()
    ...
    async def get_leave_balances(self, employee_id: str) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, headers=headers)
            if resp.status_code >= 400:
                return {"error": f"Upstream HTTP {resp.status_code}", "balances": []}
            data = resp.json()
