...
def resolve_department_agent(
    ...
    if best is None or best.score <= 0.05:
        best = ResolvedAgent(
            ...
            department="general",
            system_prompt="أنت HSAAI Assistant. أجب باحترافية ووضوح واعتمد على مصادر المؤسسة عند توفرها.",
    ...
    if not user_can_use_agent(claims, best):
        return ResolvedAgent(
            ...
            name="HSAAI Assistant",
            department="general",
            system_prompt="أنت HSAAI Assistant. لا تعرض معلومات غير مصرح بها. اطلب من المستخدم التواصل مع المسؤول إذا احتاج صلاحية إضافية.",
            knowledge_scopes=["general"],
            allowed_roles=["hsaai_admin", "department_manager", "ai_user", "auditor"],
            score=best.score,
