...
class KnowledgePermission(Base):
    ...
    tenant_id = Column(String(64), index=True, nullable=False, server_default="default")
    workspace_id = Column(String(64), index=True, nullable=False, server_default="default")
...
class HumanApprovalRequest(Base):
    ...
    requires_two_person = Column(Boolean, nullable=False, default=False, server_default="false")
    second_approver = Column(String(128), nullable=False, default="", server_default="")
    sla_hours = Column(Integer, nullable=False, default=24, server_default="24")
...
class DepartmentAgent(Base):
    ...
    id = Column(Integer, primary_key=True)
    # FIX (post-audit): key must be UNIQUE — department_agent_runs.agent_key
    # holds a ForeignKey to this column, and PostgreSQL requires the target of
    # a FK to be a UNIQUE/PK column (SQLite silently allowed it in dev).
