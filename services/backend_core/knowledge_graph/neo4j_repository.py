...
class Neo4jGraphRepository:
    ...
    # FIX v0.1 (P0): Whitelist of allowed relationship types to prevent
    # Cypher injection via `rel_type` interpolation. Neo4j does not support
    # parameterized relationship types, so we must validate before f-string
    # interpolation.
    ALLOWED_REL_TYPES = frozenset({
        "MENTIONS", "REFERENCES", "OWNS", "APPROVES", "DEPENDS_ON",
        "SUPPLIES_TO", "REPORTS_TO", "PART_OF", "GOVERNS", "AFFECTS",
        "CREATES", "USES", "PRODUCES", "CONSUMES", "SIGNS", "OBLIGATES",
        "EXPOSES", "MITIGATES", "TRIGGERS", "DERIVES_FROM", "LINKED_TO",
        "CONTRACTED_WITH", "EMPLOYS", "MANAGES", "COLLABORATES_WITH",
    })
    ...
    def add_relationship(self, rel: dict[str, Any], actor: str = "system",
        ...
        # FIX v0.1 (P0): Validate rel_type against whitelist to prevent Cypher injection
        if rel_type not in self.ALLOWED_REL_TYPES:
            raise ValueError(
                f"Disallowed relationship type: '{rel_type}'. "
                f"Allowed types: {sorted(self.ALLOWED_REL_TYPES)}"
            )
        # Double-check: only allow alphanumeric + underscore (defense in depth)
