...
def overview(db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    svc = KnowledgeHubService(db, claims)
...
def create_space(payload: KnowledgeSpaceCreate, db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    return KnowledgeHubService(db, claims).create_space(payload)

@router.get("/spaces", dependencies=[Depends(require_permission("knowledge:read"))])
def list_spaces(db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    return KnowledgeHubService(db, claims).list_spaces()

@router.post("/collections", dependencies=[Depends(require_permission("knowledge:admin"))])
def create_collection(payload: KnowledgeCollectionCreate, db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    return KnowledgeHubService(db, claims).create_collection(payload)

@router.get("/collections", dependencies=[Depends(require_permission("knowledge:read"))])
def list_collections(space_key: str | None = None, db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    return KnowledgeHubService(db, claims).list_collections(space_key)

@router.post("/documents/register", dependencies=[Depends(require_permission("knowledge:upload"))])
async def register_document(payload: KnowledgeDocumentRegister, authorization: str | None = Header(default=None), db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    ...
    return KnowledgeHubService(db, claims).register_document(payload)
...
def list_documents(space_key: str | None = None, collection_key: str | None = None, claims: dict = Depends(get_current_claims), db: Session = Depends(get_db)):
    return KnowledgeHubService(db, claims).list_documents(space_key, collection_key, claims)

@router.get("/documents/pending", dependencies=[Depends(require_permission("knowledge:review"))])
def list_pending_documents(db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    return KnowledgeHubService(db, claims).list_pending_documents()

@router.get("/documents/{document_id}", dependencies=[Depends(require_permission("knowledge:read"))])
def get_document(document_id: str, db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    return KnowledgeHubService(db, claims).get_document(document_id)
...
def submit_for_review(document_id: str, payload: DocumentWorkflowRequest, claims: dict = Depends(get_current_claims), db: Session = Depends(get_db)):
    return KnowledgeHubService(db, claims).submit_for_review(document_id, claims.get("sub", "system"), payload.reason)
...
def approve_document(document_id: str, payload: DocumentWorkflowRequest, claims: dict = Depends(get_current_claims), db: Session = Depends(get_db)):
    return KnowledgeHubService(db, claims).approve_document(document_id, claims.get("sub", "system"), payload.reason)
...
def reject_document(document_id: str, payload: DocumentWorkflowRequest, claims: dict = Depends(get_current_claims), db: Session = Depends(get_db)):
    return KnowledgeHubService(db, claims).reject_document(document_id, claims.get("sub", "system"), payload.reason)
...
def archive_document(document_id: str, payload: DocumentWorkflowRequest, claims: dict = Depends(get_current_claims), db: Session = Depends(get_db)):
    return KnowledgeHubService(db, claims).archive_document(document_id, claims.get("sub", "system"), payload.reason)
...
def delete_document(document_id: str, claims: dict = Depends(get_current_claims), db: Session = Depends(get_db)):
    return KnowledgeHubService(db, claims).delete_document(document_id, claims.get("sub", "system"))

@router.get("/documents/{document_id}/audit", dependencies=[Depends(require_permission("audit:read"))])
def audit_trail(document_id: str, db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    return KnowledgeHubService(db, claims).audit_trail(document_id)

@router.post("/permissions", dependencies=[Depends(require_permission("knowledge:admin"))])
def grant_permission(payload: KnowledgePermissionGrant, db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    return KnowledgeHubService(db, claims).grant_permission(payload)

@router.get("/permissions", dependencies=[Depends(require_permission("knowledge:read"))])
def permissions(resource_key: str | None = None, db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    return KnowledgeHubService(db, claims).permissions(resource_key)
...
def search(payload: KnowledgeSearchRequest, claims: dict = Depends(get_current_claims), db: Session = Depends(get_db)):
    return KnowledgeHubService(db, claims).search_metadata(payload, claims)
...
def analytics(db: Session = Depends(get_db), claims: dict = Depends(get_current_claims)):
    return KnowledgeHubService(db, claims).analytics()
