...
from common.security.request_policy import verified_scope, tenant_context, workspace_context, authorization_context
...
async def _verify_with_keycloak_jwks_async(token: str) -> dict[str, Any]:
    ...
    options = {"verify_aud": VERIFY_KEYCLOAK_AUDIENCE, "require": ["exp", "iss", "aud", "sub"]}
    ...
    return jwt.decode(token, key=jwt.PyJWK.from_dict(key).key, algorithms=["RS256"], **kwargs)
...
def _roles_from_resource_access(resource_access: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(resource_access, dict):
        return roles
    allowed_clients = {x.strip() for x in os.getenv("HSAAI_ROLE_CLIENT_IDS", os.getenv("KEYCLOAK_AUDIENCE", "hsaai-api")).split(",") if x.strip()}
    for client_id, data in resource_access.items():
...
async def verify_authorization(authorization: str | None) -> dict[str, Any]:
    ...
    authorization_context.set(authorization)
    return normalized
