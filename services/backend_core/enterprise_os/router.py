...
def _runtime_error(name: str, exc: Exception) -> HTTPException:
    ...
    return HTTPException(status_code=503, detail=f"{name} runtime dependency is not available")
...
def _get_db_dependency() -> Callable[..., Any]:
    try:
        ...
        error = str(exc)
        def unavailable_db():
            raise HTTPException(503, "Database runtime dependency is unavailable")
...
def _permission_dependency(permission: str) -> Callable[..., Any]:
    try:
        ...
        def deny_unavailable_permission():
            raise HTTPException(503, "Authorization dependency unavailable")
        return deny_unavailable_permission
...
class ProjectCreate(BaseModel):
    ...
    expected_roi: float = Field(default=0.0, allow_inf_nan=False)
    cost_estimate: float = Field(default=0.0, ge=0, allow_inf_nan=False)
    risk_level: str = "medium"
    description: str = ""
...
class CostCreate(BaseModel):
    ...
    vector_storage_cost: float = Field(default=0.0, ge=0, allow_inf_nan=False)
    compute_cost: float = Field(default=0.0, ge=0, allow_inf_nan=False)
...
def _claims_scope(claims: dict[str, Any]) -> tuple[str, str, str]:
    try:
        ...
        if not isinstance(actor, str) or not actor.strip():
            raise ValueError("Token requires a subject")
        return tenant, workspace, actor
    except ValueError as exc:
        raise HTTPException(403, str(exc)) from exc


def _audit(db: Any, actor: str, action: str, resource: str, workspace_id: str = "default", success: bool = True, tenant_id: str = "default") -> None:
    m = _models()
    db.add(m["AuditLog"](actor=actor, action=action, resource=resource, workspace_id=workspace_id, tenant_id=tenant_id, success=success))


def _scoped_query(db, model, claims):
    tenant, workspace, _ = _claims_scope(claims)
    return db.query(model).filter(model.tenant_id == tenant, model.workspace_id == workspace)


def _default_agent_key(key, claims):
    tenant, workspace, _ = _claims_scope(claims)
    if (tenant, workspace) == ("default", "default"):
        return key
    prefix = hashlib.sha256(f"{tenant}:{workspace}".encode()).hexdigest()[:12]
    return f"{prefix}-{key}"


def _ensure_default_agents(db: Any, claims: dict) -> None:
    m = _models()
    ...
    for key, name, department, description in DEFAULT_AGENTS:
        key = _default_agent_key(key, claims)
        if not _scoped_query(db, EnterpriseAgent, claims).filter(EnterpriseAgent.agent_key == key).first():
            db.add(
                EnterpriseAgent(
                    agent_key=key,
                    name=name,
                    ...
                    tenant_id=tenant, workspace_id=workspace, created_by=actor,
...
def _agent_to_dict(a: Any) -> dict[str, Any]:
    return {
        ...
        "tools": _safe_json(getattr(a, "tools", []), []),
        "knowledge_sources": _safe_json(getattr(a, "knowledge_sources", []), []),
        "permissions": _safe_json(getattr(a, "permissions", []), []),
...
def list_agents(db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); EnterpriseAgent = m["EnterpriseAgent"]
    _ensure_default_agents(db, claims)
    return {"items": [_agent_to_dict(a) for a in _scoped_query(db, EnterpriseAgent, claims).order_by(EnterpriseAgent.department, EnterpriseAgent.name).all()]}
...
def create_agent(payload: AgentUpsert, claims: dict[str, Any] = Depends(get_claims_dep), db: Any = Depends(get_db_dep)):
    ...
    db.add(agent); _audit(db, actor, "agent.create", payload.agent_key, workspace, tenant_id=tenant); db.commit(); db.refresh(agent)
    return _agent_to_dict(agent)


@router.get("/agents/{agent_key}", dependencies=[Depends(_permission_dependency("agents:read"))])
def get_agent(agent_key: str, db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); EnterpriseAgent = m["EnterpriseAgent"]
    agent = _scoped_query(db, EnterpriseAgent, claims).filter(EnterpriseAgent.agent_key == agent_key).first()
    if not agent:
...
def update_agent(agent_key: str, payload: AgentUpsert, claims: dict[str, Any] = Depends(get_claims_dep), db: Any = Depends(get_db_dep)):
    ...
    agent.knowledge_sources = payload.knowledge_sources
    agent.permissions = payload.permissions
    agent.version += 1
    _audit(db, actor, "agent.update", agent_key, workspace, tenant_id=tenant); db.commit(); db.refresh(agent)
    return _agent_to_dict(agent)
...
def run_agent(payload: dict[str, Any], claims: dict[str, Any] = Depends(get_claims_dep), db: Any = Depends(get_db_dep)):
    ...
    scoped = _scoped_query(db, EnterpriseAgent, claims)
    agent = scoped.filter(EnterpriseAgent.agent_key.in_([selected, _default_agent_key(selected, claims)]), EnterpriseAgent.enabled.is_(True)).first()  # noqa: E712
    if not agent:
    ...
    output = ""  # Routing is not model or tool execution.
    approval = None
    if needs_approval:
        ...
        approval = ApprovalRequest(
            ...
            status="pending",
            required_roles=_approval_chain(_risk_level(action_type, message), agent.department),
            requested_by=actor,
            tenant_id=tenant,
            workspace_id=workspace,
        )
    ...
    db.add(AgentLog(request_id=request_id, user_id=actor, agent_key=agent.agent_key, action=action_type, input_text=message, output_text=output, confidence=confidence, latency_ms=int((time.time() - started) * 1000), success=False, tenant_id=tenant, workspace_id=workspace))
    _audit(db, actor, "agent.run", agent.agent_key, workspace, tenant_id=tenant); db.commit()
    return {"request_id": request_id, "supervisor": {"selected_agent": agent.agent_key, "confidence": confidence, "multi_agent_ready": False}, "agent": _agent_to_dict(agent), "requires_approval": needs_approval, "approval_id": approval.approval_id if approval else None, "answer": output, "status": "awaiting_approval" if needs_approval else "routed", "executed": False}
...
def list_approvals(status: str | None = None, db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); ApprovalRequest = m["ApprovalRequest"]
    q = _scoped_query(db, ApprovalRequest, claims)
    if status:
        q = q.filter(ApprovalRequest.status == status)
    items = []
    for a in q.order_by(ApprovalRequest.id.desc()).all():
        d = _clean_dict(a); d["payload"] = _safe_json(getattr(a, "payload", {}), {}); d["required_roles"] = _safe_json(getattr(a, "required_roles", []), []); items.append(d)
    return {"items": items}
...
def create_approval(payload: ApprovalCreate, claims: dict[str, Any] = Depends(get_claims_dep), db: Any = Depends(get_db_dep)):
    ...
    effective_risk = max([payload.risk_level, _risk_level(payload.action_type)], key=risk_order.get)
    item = ApprovalRequest(approval_id=approval_id, title=payload.title, action_type=payload.action_type, resource_type=payload.resource_type, resource_id=payload.resource_id, recommendation=payload.recommendation, payload=payload.payload, risk_level=effective_risk, required_roles=payload.required_roles, sla_hours=payload.sla_hours, requested_by=actor, tenant_id=tenant, workspace_id=workspace)
    db.add(item); _audit(db, actor, "approval.create", approval_id, workspace, tenant_id=tenant); db.commit()
    return {"approval_id": approval_id, "status": "pending"}


def _review_request(db, approval_id, claims):
    from backend_core.security.rbac import _roles_from_claims
    models = _models()
    item = _scoped_query(db, models["ApprovalRequest"], claims).filter(
        models["ApprovalRequest"].approval_id == approval_id).with_for_update().first()
    if item is None:
        raise HTTPException(404, "Approval not found")
    if item.status != "pending":
        raise HTTPException(409, "Approval already decided")
    actor = _claims_scope(claims)[2]
    if actor == item.requested_by:
        raise HTTPException(403, "Requester cannot review their own action")
    chain = list(item.required_roles or ["department_manager"])
    if item.risk_level in {"high", "critical"} and len(chain) < 2:
        chain = chain * 2
    step = (item.current_step or 1) - 1
    if step >= len(chain):
        raise HTTPException(409, "Approval chain is already complete")
    roles = set(_roles_from_claims(claims))
    aliases = {"reviewer": "document_reviewer", "manager": "department_manager", "admin": "hsaai_admin"}
    required = aliases.get(chain[step], chain[step])
    if required not in roles and "hsaai_admin" not in roles:
        raise HTTPException(403, "Current approval step requires another role")
    History = models["ApprovalHistory"]
    if db.query(History).filter_by(approval_id=approval_id, actor=actor).first():
        raise HTTPException(403, "Each approval step requires an independent reviewer")
    return item, chain
...
def approve(approval_id: str, payload: DecisionPayload, claims: dict[str, Any] = Depends(get_claims_dep), db: Any = Depends(get_db_dep)):
    item, chain = _review_request(db, approval_id, claims)
    tenant, workspace, actor = _claims_scope(claims)
    History = _models()["ApprovalHistory"]
    db.add(History(approval_id=approval_id, step_no=item.current_step, actor=actor, decision="approved", comment=payload.comment))
    item.reviewed_by = actor
    if item.current_step >= len(chain):
        item.status = "approved"
    else:
        item.current_step += 1
    _audit(db, actor, "approval.approve", approval_id, workspace, tenant_id=tenant)
    db.commit()
    return {"approval_id": approval_id, "status": item.status, "execution": "pending_executor", "executed": False}
...
def reject(approval_id: str, payload: DecisionPayload, claims: dict[str, Any] = Depends(get_claims_dep), db: Any = Depends(get_db_dep)):
    item, _ = _review_request(db, approval_id, claims)
    tenant, workspace, actor = _claims_scope(claims)
    History = _models()["ApprovalHistory"]
    item.status = "rejected"
    item.reviewed_by = actor
    item.reject_reason = payload.comment
    db.add(History(approval_id=approval_id, step_no=item.current_step, actor=actor, decision="rejected", comment=payload.comment))
    _audit(db, actor, "approval.reject", approval_id, workspace, tenant_id=tenant)
    db.commit()
    return {"approval_id": approval_id, "status": "rejected"}


@router.get("/knowledge-graph", dependencies=[Depends(_permission_dependency("knowledge:read"))])
def graph(db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); KnowledgeEntity = m["KnowledgeEntity"]; KnowledgeRelationship = m["KnowledgeRelationship"]
    return {"entities": [_clean_dict(e) for e in _scoped_query(db, KnowledgeEntity, claims).limit(200).all()], "relationships": [_clean_dict(r) for r in _scoped_query(db, KnowledgeRelationship, claims).limit(500).all()]}
...
def create_entity(payload: EntityCreate, claims: dict[str, Any] = Depends(get_claims_dep), db: Any = Depends(get_db_dep)):
    ...
    entity = KnowledgeEntity(entity_key=key, name=payload.name, entity_type=payload.entity_type, description=payload.description, source_ref=payload.source_ref, confidence=payload.confidence, classification=payload.classification, extra_metadata=payload.metadata, tenant_id=tenant, workspace_id=workspace)
    db.add(entity); _audit(db, actor, "kg.entity.create", key, workspace, tenant_id=tenant); db.commit()
...
def create_relationship(payload: RelationshipCreate, claims: dict[str, Any] = Depends(get_claims_dep), db: Any = Depends(get_db_dep)):
    ...
    tenant, workspace, actor = _claims_scope(claims)
    Entity = m["KnowledgeEntity"]
    for key in (payload.source_key, payload.target_key):
        if not _scoped_query(db, Entity, claims).filter(Entity.entity_key == key).first():
            raise HTTPException(404, "Relationship entity not found in scope")
    rel = KnowledgeRelationship(source_key=payload.source_key, relationship_type=payload.relationship_type, target_key=payload.target_key, source_ref=payload.source_ref, confidence=payload.confidence, extra_metadata=payload.metadata, tenant_id=tenant, workspace_id=workspace)
    db.add(rel); _audit(db, actor, "kg.relationship.create", f"{payload.source_key}->{payload.target_key}", workspace, tenant_id=tenant); db.commit()
...
def enterprise_search(payload: SearchRequest, claims: dict[str, Any] = Depends(get_claims_dep), db: Any = Depends(get_db_dep)):
    ...
    started = time.time(); tenant, workspace, actor = _claims_scope(claims)
    entities = _scoped_query(db, KnowledgeEntity, claims).filter(KnowledgeEntity.name.ilike(f"%{payload.query}%")).limit(10).all()
...
def coe_projects(db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); AICoEProject = m["AICoEProject"]
    return {"items": [_clean_dict(p) for p in _scoped_query(db, AICoEProject, claims).order_by(AICoEProject.id.desc()).all()]}


@router.post("/coe/projects", dependencies=[Depends(_permission_dependency("reports:write"))])
def create_project(payload: ProjectCreate, db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); AICoEProject = m["AICoEProject"]
    data = payload.model_dump() if hasattr(payload, "model_dump") else payload.dict()
    tenant, workspace, actor = _claims_scope(claims)
    p = AICoEProject(**data, tenant_id=tenant, workspace_id=workspace)
    db.add(p)
    _audit(db, actor, "coe.project.create", p.project_key, workspace, tenant_id=tenant)
    try:
        db.commit()
    except IntegrityError as exc:
        db.rollback()
        raise HTTPException(409, "Project key is unavailable") from exc
...
def coe_policies(db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); AIPolicy = m["AIPolicy"]
    return {"items": [_clean_dict(p) for p in _scoped_query(db, AIPolicy, claims).all()]}


@router.get("/coe/risks", dependencies=[Depends(_permission_dependency("audit:read"))])
def coe_risks(db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); AIRisk = m["AIRisk"]
    return {"items": [_clean_dict(r) for r in _scoped_query(db, AIRisk, claims).all()]}


@router.get("/coe/training", dependencies=[Depends(_permission_dependency("reports:read"))])
def coe_training(db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); AITraining = m["AITraining"]
    return {"items": [_clean_dict(t) for t in _scoped_query(db, AITraining, claims).all()]}


@router.get("/finops/usage", dependencies=[Depends(_permission_dependency("analytics:read"))])
def finops_usage(db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); CostRecord = m["CostRecord"]
    query = _scoped_query(db, CostRecord, claims)
    total_tokens, total_cost = query.with_entities(func.sum(CostRecord.tokens), func.sum(CostRecord.total_cost)).one()
    records = query.order_by(CostRecord.id.desc()).limit(100).all()
    return {"total_tokens": total_tokens or 0, "total_cost": round(total_cost or 0.0, 4), "records": [_clean_dict(r) for r in records]}
...
def record_cost(payload: CostCreate, claims: dict[str, Any] = Depends(get_claims_dep), db: Any = Depends(get_db_dep)):
    ...
    r.tenant_id, r.workspace_id = tenant, workspace
    db.add(r); _audit(db, actor, "finops.cost.record", r.record_key, workspace, tenant_id=tenant); db.commit()
...
def list_integrations(db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); Integration = m["Integration"]
    return {"items": [_clean_dict(i) for i in _scoped_query(db, Integration, claims).order_by(Integration.system_type, Integration.name).all()]}
...
def create_integration(payload: IntegrationCreate, claims: dict[str, Any] = Depends(get_claims_dep), db: Any = Depends(get_db_dep)):
    m = _models(); Integration = m["Integration"]
    tenant, workspace, actor = _claims_scope(claims)
    if _scoped_query(db, Integration, claims).filter(Integration.connector_key == payload.connector_key).first():
        raise HTTPException(409, "Connector key already exists")
    item = Integration(**payload.model_dump(), tenant_id=tenant, workspace_id=workspace, health_status="not_configured", sync_status="never")
    db.add(item)
    _audit(db, actor, "integration.create", payload.connector_key, workspace, tenant_id=tenant)
    try:
        db.commit()
    except IntegrityError as exc:
        db.rollback()
        raise HTTPException(409, "Connector key is unavailable") from exc
...
def test_integration(connector_key: str, db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); Integration = m["Integration"]; ConnectorLog = m["ConnectorLog"]
    tenant, workspace, actor = _claims_scope(claims)
    item = _scoped_query(db, Integration, claims).filter(Integration.connector_key == connector_key).first()
    if not item:
        raise HTTPException(404, "Integration not found")
    status = "not_tested" if item.enabled and item.base_url else "not_configured"
    item.health_status = status
    db.add(ConnectorLog(connector_key=connector_key, action="test_connection", success=False, message=status, tenant_id=tenant, workspace_id=workspace))
    db.commit()
    return {"connector_key": connector_key, "health_status": status, "connected": False, "validation": "configuration_only", "message": "Connection not tested: tenant credential binding and an authenticated provider probe are required."}
...
@router.get("/executive/command-center", dependencies=[Depends(_permission_dependency("analytics:read"))])
def executive_command_center(db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    """Read verified-scope operational metrics; outages are not zero usage."""
    from backend_core.db.models import LLMUsageLog, AICostRecord, ExecutiveMetric
    try:
        total_queries = _scoped_query(db, LLMUsageLog, claims).count()
        total_cost = _scoped_query(db, AICostRecord, claims).with_entities(func.sum(AICostRecord.total_cost)).scalar() or 0.0
        active_agents = _scoped_query(db, ExecutiveMetric, claims).filter(
            ExecutiveMetric.metric_key == "active_agents"
        ).order_by(ExecutiveMetric.created_at.desc(), ExecutiveMetric.id.desc()).first()
        active_agent_count = active_agents.metric_value if active_agents else 0
    except SQLAlchemyError as exc:
        raise _runtime_error("Executive metrics database", exc) from exc
...
@router.get("/integrations/catalog", dependencies=[Depends(_permission_dependency("integrations:read"))])
def integrations_catalog(db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    """Expose scoped registrations without probing global credentials."""
    Integration = _models()["Integration"]
    connectors = []
    for item in _scoped_query(db, Integration, claims).order_by(Integration.name).all():
        connectors.append({"name": item.name, "connector_key": item.connector_key,
            "system_type": item.system_type, "auth_type": item.auth_type,
            "mode": "configured" if item.enabled else "disabled",
            "connected": False, "health_status": "not_tested",
            "validation": "authenticated_provider_probe_required"})
...
def audit_logs(limit: int = 100, db: Any = Depends(get_db_dep), claims: dict = Depends(get_claims_dep)):
    m = _models(); AuditLog = m["AuditLog"]
    return {"items": [_clean_dict(a) for a in _scoped_query(db, AuditLog, claims).order_by(AuditLog.id.desc()).limit(max(1, min(limit, 1000))).all()]}
