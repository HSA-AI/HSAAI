...
class AgentMarketplace:
    ...
    def list_agents(
        ...
        if status:
            result = [a for a in result if any(v.status == status for v in a.versions)]
        return result
    ...
    def promote_to_production(
        ...
        if prev_prod and prev_prod.version_id != version_id and not canary:
    ...
    def increase_canary(self, agent_id: str, version_id: str, percentage: int) -> AgentVersion:
        ...
        if percentage == 100:
            for previous in agent.versions:
                if previous.status == AgentStatus.PRODUCTION and previous.version_id != version_id:
                    previous.status = AgentStatus.RETIRED
            version.status = AgentStatus.PRODUCTION
            agent.current_version = version_id
        self._save()
    ...
    def rollback(self, agent_id: str) -> Optional[AgentVersion]:
        ...
        if not agent:
            raise ValueError(f"Agent not found: {agent_id}")
        canaries = [v for v in agent.versions if v.status == AgentStatus.CANARY]
        stable = agent.get_production_version()
        if canaries:
            for version in canaries:
                version.status = AgentStatus.RETIRED
                version.canary_percentage = 0
            agent.current_version = stable.version_id if stable else ""
            self._save()
            return stable
    ...
    def update_metrics(self, agent_id: str, version_id: str, metrics_update: dict):
        ...
        if not version:
            return
        # Counters are additive; averages describe the supplied execution batch.
        previous_count = max(0, version.metrics.get("total_executions", 0))
        batch_count = max(0, metrics_update.get("total_executions", 0))
        for key, value in metrics_update.items():
            if key in {"avg_satisfaction", "avg_latency_ms"}:
                if batch_count:
                    version.metrics[key] = (version.metrics.get(key, 0.0) * previous_count + value * batch_count) / (previous_count + batch_count)
                continue
        ...
        if total > 100 and version.status in {AgentStatus.PRODUCTION, AgentStatus.CANARY}:  # evaluate the active version only
