...
def _cache_key(prompt: str, system: str, model: str, temperature: float, max_tokens: int = 1024) -> str:
    """Build a deterministic cache key from the request parameters.

    Preserve exact prompt text and generation settings so distinct requests
    cannot share a cached result accidentally.
    """
    normalized = json.dumps([system, prompt, model, temperature, max_tokens], ensure_ascii=False, separators=(",", ":"))
    return hashlib.sha256(normalized.encode()).hexdigest()


def _check_semantic_cache(prompt: str, system: str, model: str, temperature: float, tenant_id: str, max_tokens: int = 1024) -> Optional[dict]:
    ...
    if client is None:
        return None
    try:
        key = f"llm_cache:{tenant_id}:{_cache_key(prompt, system, model, temperature, max_tokens)}"
...
def _store_in_semantic_cache(prompt: str, system: str, model: str, temperature: float,
                              tenant_id: str, response: dict, ttl: int = 86400, max_tokens: int = 1024):
...
def _check_token_budget(tenant_id: str, estimated_tokens: int) -> tuple[bool, int, int]:
    ...
    if client is None:
        if os.getenv("APP_ENV") in {"production", "prod", "staging"}:
            raise HTTPException(503, "Token budget storage unavailable")
    ...
    try:
        ...
        if os.getenv("APP_ENV") in {"production", "prod", "staging"}:
            raise HTTPException(503, "Token budget storage unavailable") from e
        logger.warning("Token budget check failed: %s", e)
        return True, 0, DEFAULT_DAILY_TOKEN_BUDGET
...
def _estimate_tokens(text: str) -> int:
    """FIX v0.1 (P0): Real token counting via tiktoken when available,
    with heuristic fallback. Previously used char-count heuristic with
    ±50% accuracy — caused budget enforcement to be unreliable."""
    # Try tiktoken first (accurate for Qwen/GPT-like tokenizers)
    try:
        import tiktoken
        if not hasattr(_estimate_tokens, "_enc"):
            # cl100k_base is a reasonable approximation for Qwen3
            _estimate_tokens._enc = tiktoken.get_encoding("cl100k_base")
        return len(_estimate_tokens._enc.encode(text))
    except ImportError:
        pass
    except Exception:
        pass
    # Fallback: heuristic (~4 chars/token English, ~2 chars/token Arabic)
...
async def ollama_generate(req: GenerateRequest) -> str:
    """FIX v0.1 (P0): Switch from /api/generate (raw prompt concat) to
    /api/chat with proper messages array. Previously built:
        f"{req.system}\\n\\nUser: {req.prompt}\\nAssistant:"
    which broke Qwen3's ChatML template, lost system-prompt priority, and
    increased prompt-injection success. Now uses chat-format messages."""
    model = route_model(req.prompt, req.task, req.sensitivity, req.model)["model"]
    payload = {
        "model": model,
        "messages": [
            {"role": "system", "content": req.system or "You are HSAAI, a helpful enterprise assistant."},
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        ...
        if r.status_code >= 400:
            raise HTTPException(r.status_code, f"Provider request failed (HTTP {r.status_code})")
        data = r.json()
...
async def external_generate(req: GenerateRequest, selected: dict[str, Any]) -> str:
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        ...
        if provider == "google":
            ...
            # SECURITY FIX (audit P2-11): the API key was passed as a URL query
            # parameter (?key=…), which leaks it into proxy/access logs. Google's
            # API accepts the key via the x-goog-api-key header instead.
            r = await client.post(
...
async def ollama_stream(req: GenerateRequest) -> AsyncIterator[bytes]:
    ...
    # FIX v0.1 (P0): Use /api/chat with messages array (not /api/generate
    # with raw prompt concat) — fixes ChatML template + system priority.
    model_name = route_model(req.prompt, req.task, req.sensitivity, req.model)["model"]
    payload = {
        "model": model_name,
        "messages": [
            {"role": "system", "content": req.system or "You are HSAAI, a private enterprise AI assistant."},
            {"role": "user", "content": req.prompt},
        ],
        "stream": True,
        "options": {"temperature": req.temperature, "num_predict": req.max_tokens},
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(120.0, connect=10.0)) as client:
            # FIX v0.1 (P0): Use /api/chat (not /api/generate) for proper ChatML
            async with client.stream("POST", f"{OLLAMA_BASE_URL}/api/chat", json=payload) as r:
                if r.status_code >= 400:
                    yield f"data: {json.dumps({'error': f'Ollama HTTP {r.status_code}'}, ensure_ascii=False)}\n\n".encode()
                    yield b"data: [DONE]\n\n"
                    return
                async for line in r.aiter_lines():
                    ...
                    try:
                        ...
                        continue
                    # FIX v0.1 (P0): /api/chat returns {"message": {"content": "..."}}
                    # while /api/generate returned {"response": "..."}. Support both.
        ...
        logger.warning("Stream upstream error: %s", type(exc).__name__)
        yield f"data: {json.dumps({'error': 'upstream_unavailable'}, ensure_ascii=False)}\n\n".encode()
...
async def generate(req: GenerateRequest, claims: dict = Depends(_auth_dep)):
    ...
    cache_scope = hashlib.sha256(json.dumps([req.tenant_id, req.workspace_id, claims.get("sub", "")]).encode()).hexdigest()
    cached = _check_semantic_cache(req.prompt, req.system, selected["model"], req.temperature, cache_scope, req.max_tokens)
    ...
    _store_in_semantic_cache(
        ...
        cache_scope, {"text": text, "provider": provider}, ttl=86400, max_tokens=req.max_tokens,
    )
