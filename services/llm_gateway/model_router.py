...
def route_model(prompt: str, task: str | None = None, sensitivity: str | None = None, requested_model: str | None = None) -> dict[str, Any]:
    ...
    if pinned and os.getenv("ENABLE_MULTI_MODEL_ROUTING", "false").lower() != "true":
        return {"provider": "ollama", "model_key": "configured", "model": pinned, "reason": "configured-local-model", "local_only": True}
