...
def _load_model():
    ...
    """
    global _model, _model_load_error, _current_vector_size, VECTOR_SIZE
    with _model_lock:
...
def set_model(model_name: str, *, version: Optional[str] = None) -> None:
    ...
    """
    global _current_model_name, _current_model_version, _model, _model_load_error, VECTOR_SIZE
    if not model_name or not isinstance(model_name, str):
        raise ValueError("model_name must be a non-empty string")

    with _model_lock:
        if model_name == _current_model_name and _model is not None and version in {None, _current_model_version}:
...
def _serialize_model_access(function):
    @wraps(function)
...
def _cache_namespace():
    return f"{_current_model_name}:{_current_model_version}"


def _validate_vector(vector):
    values = [float(value) for value in vector]
    if len(values) != _current_vector_size or not all(math.isfinite(v) for v in values) or not any(values):
        raise RuntimeError("Invalid embedding vector; indexing aborted")
    return values


@_serialize_model_access
def embed_text(text: str) -> list[float]:
    ...
    if text is None:
        text = ""
    # Cache lookup.
    cached = _cache.get(_cache_namespace(), text)
    ...
    out = _validate_vector(vec)
    _cache.put(_cache_namespace(), text, out)
    return out
    ...
    try:
        ...
        for t in miss_texts:
            try:
                ...
                raise RuntimeError("Embedding failed; indexing aborted") from e2

    # 3. Store misses in cache + splice into the results list.
    if len(raw_vectors) != len(miss_indices):
        raise RuntimeError("Embedding batch size mismatch; indexing aborted")
    validated = [_validate_vector(vector) for vector in raw_vectors]
    for idx, text, out in zip(miss_indices, miss_texts, validated):
        results[idx] = out
        _cache.put(_cache_namespace(), text, out)
