...
try:
    ...
    _fallback = Path(os.getenv("HSAAI_HOME", Path.cwd())) / "data" / "local_uploads"
    _fallback.mkdir(parents=True, exist_ok=True)
    STORAGE = _fallback
    logging.getLogger("hsaai.rag").warning(
        "LOCAL_FILE_STORAGE unwritable — falling back to %s", _fallback
    )
...
if not os.access(EVENT_DB_PATH.parent, os.W_OK):
    # PORTABILITY FIX: fall back to a writable location outside Docker.
    EVENT_DB_PATH = Path(os.getenv("HSAAI_HOME", str(Path.cwd()))) / "data" / "rag_events.db"
...
from common.security.request_policy import authorization_context, outgoing_headers

async def protect_api(request: Request):
    if request.url.path in {"/health", "/ready", "/health/auth"}:
        yield
        return
    auth = request.headers.get("authorization", "")
    if not auth.lower().startswith("bearer "):
        raise HTTPException(401, "Bearer authentication required")
    claims = await run_in_threadpool(verify_jwt, auth.split(" ", 1)[1])
    try:
        verified_scope(claims)
    except ValueError as exc:
        raise HTTPException(403, str(exc)) from exc
    request.state.claims = claims
    context_token = authorization_context.set(auth)
    try:
        yield
    finally:
        authorization_context.reset(context_token)

app = FastAPI(title="HSAAI RAG Engine", version=APP_VERSION, dependencies=[Depends(protect_api)])
...
async def upload_document(
    ...
    """
    tenant_id, workspace_id = verified_scope(request.state.claims)
...
def search(req: SearchRequest, claims: dict = Depends(_auth_dep)):
    ...
    if client:
        ...
        # FIX v0.1 (P1): Use query_points() instead of deprecated search().
        # client.search() is deprecated in Qdrant 1.10+ and will be removed.
        try:
            response = client.query_points(
                collection_name=COLLECTION,
                query=vector,
                query_filter=flt,
                limit=max(req.top_k * 4, 20),
                with_payload=True,
            )
...
async def answer(req: AnswerRequest, claims: dict = Depends(_auth_dep)):
    ...
    if RAG_ANSWER_USE_LLM:
        try:
            async with httpx.AsyncClient(timeout=120, headers=outgoing_headers()) as client:
    ...
    answer_type = "llm_grounded" if used_llm else "retrieval_fallback"
...
def highlight(req: HighlightRequest, claims: dict = Depends(_auth_dep)):
    req = _scope_request(req, claims)
    hits = search(req, claims)["results"]
...
async def answer_stream(req: AnswerRequest, claims: dict = Depends(_auth_dep)):
    req = _scope_request(req, claims)
    """FIX v0.1 (P0): Real streaming — previously awaited full answer() then
    re-emitted context word-by-word (fake streaming with TTFT = full
    generation latency). Now streams tokens directly from llm_gateway
    /v1/stream SSE endpoint, passing through to the client with proper
    cancellation support."""
    from fastapi.responses import StreamingResponse
    import json, asyncio, httpx

    # Step 1: Run search synchronously to get context (fast, &lt;500ms)
    search_req = SearchRequest(**{name: getattr(req, name) for name in SearchRequest.model_fields})
    search_payload = await run_in_threadpool(search, search_req, claims)
    results = search_payload.get("results", [])
    context_chunks = [r.get("text", "")[:1500] for r in results[:5] if r.get("text")]
    context = "\n\n---\n\n".join(context_chunks) if context_chunks else "No context found."

    # Build the prompt
    system_prompt = (
        "You are HSAAI, a private enterprise RAG assistant. "
        "Answer only from the provided context. If the context is insufficient, "
        "say that clearly. Use Arabic by default. Cite source numbers like [1], [2]."
    )
    user_prompt = f"Context:\n{context}\n\nQuestion: {req.query}\n\nAnswer:"
    from common.prompt_security import sanitize_user_query, sanitize_rag_context, build_safe_prompt, should_block_request
    query_result = sanitize_user_query(req.query)
    if should_block_request(query_result.risk_score):
        raise HTTPException(400, "Query rejected by prompt security policy")
    chunks, _warnings = sanitize_rag_context([{"text": value} for value in context_chunks])
    context = "\n\n".join(f"[{i+1}] {chunk['text']}" for i, chunk in enumerate(chunks))
    user_prompt = build_safe_prompt(system_prompt=system_prompt, rag_context=context, user_query=query_result.sanitized)
    ...
    async def events():
        ...
        yield "data: " + json.dumps({
            "sources": [{"index": i+1, "doc_id": item.get("doc_id"), "filename": item.get("filename"), "page": item.get("page")} for i, item in enumerate(results[:5])],
            "features": search_payload.get("features", []),
            "context_chunks": len(context_chunks),
        }, ensure_ascii=False) + "\n\n"

        # Stream tokens from llm_gateway
        llm_url = os.getenv("LLM_GATEWAY_URL", "http://llm_gateway:8090")
        try:
            async with httpx.AsyncClient(timeout=120.0, headers=outgoing_headers()) as client:
                async with client.stream(
                    ...
                    async for line in resp.aiter_lines():
                        if not line:
                            yield "\n"
                            continue
                        # Pass through SSE events from llm_gateway
                        if line.startswith("event:") or line.startswith("data:"):
                            yield line + "\n"
                        elif line.strip():
                            # Treat as token data
                            yield "event: token\n"
                            yield "data: " + json.dumps({"token": line}, ensure_ascii=False) + "\n\n"
        except httpx.ReadError:
            yield "event: error\ndata: " + json.dumps({"error": "stream interrupted"}) + "\n\n"
        except Exception as e:
            yield "event: error\ndata: " + json.dumps({"error": "Language model unavailable"}, ensure_ascii=False) + "\n\n"
...
def _scope_request(req, claims):
    tenant_id, workspace_id = verified_scope(claims)
    values = {"tenant_id": tenant_id, "workspace_id": workspace_id,
              "user_id": claims.get("sub", ""),
              "user_roles": claims.get("roles") or claims.get("realm_access", {}).get("roles", [])}
    return req.model_copy(update={k: v for k, v in values.items() if k in type(req).model_fields})
