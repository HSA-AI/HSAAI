...
def _try_ocr_image(raw: bytes, lang: str = "ara+eng") -> str:
    ...
    try:
        ...
        return pytesseract.image_to_string(image, lang=lang, timeout=30) or ""
    except Exception:
        return ""
...
def _try_ocr_pdf(raw: bytes, lang: str = "ara+eng", max_pages: int = 25) -> str:
    ...
    try:
        ...
        import pytesseract
        pages = convert_from_bytes(raw, dpi=220, first_page=1, last_page=max_pages, timeout=120)
        texts = []
        for index, image in enumerate(pages, start=1):
            text = pytesseract.image_to_string(image, lang=lang, timeout=30) or ""
            if text.strip():
