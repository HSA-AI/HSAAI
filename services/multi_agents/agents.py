...
class BaseAgent:
    ...
    async def _get_rag_context(self, query: str, tenant_id: str, workspace_id: str) -> tuple[str, int]:
        ...
        try:
            async with httpx.AsyncClient(timeout=20, headers=outgoing_headers()) as client:
...
import json
_prompt_file = Path(__file__).with_name("prompts.v1.json")
for _key, _definition in json.loads(_prompt_file.read_text(encoding="utf-8"))["agents"].items():
    AGENTS.setdefault(_key, SpecialistAgent(_key, _definition))
