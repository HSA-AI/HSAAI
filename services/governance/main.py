...
class ComplianceFramework(str, Enum):
    ...
    # FIX v0.1 (P0): Restore NDMO (Yemen/Saudi Data Management Office) —
    # was silently dropped in v5.0 when compliance_reports service was deleted.
    # Critical for HSA Group (Yemeni conglomerate).
    NDMO             = "ndmo"             # National Data Management Office (Yemen/KSA)
...
async def check_access(
    ...
    SECURITY FIX v0.1 (P0): Previously accepted `subject` dict from client
    request body, allowing any caller to forge `{"role": "super_admin",
    "tenant_id": "any"}` and receive `allowed: True`. Now `subject.role`,
    `subject.tenant_id`, `subject.user_id` are sourced from the verified
    JWT `claims`, ignoring client-supplied subject identity fields.
    """
    # FIX v0.1 (P0): Override client-supplied subject identity fields with
    # values from the verified JWT claims. Only allow non-identity fields
    # (e.g., department, clearance_level) from the client payload.
    s = Subject(**subject)
    s.user_id = claims.get("sub") or claims.get("preferred_username") or s.user_id
    s.tenant_id = claims.get("tenant_id", s.tenant_id)
    jwt_roles = claims.get("roles") or claims.get("realm_access", {}).get("roles", [])
    if jwt_roles:
        s.role = jwt_roles[0]  # primary role from JWT
    r = Resource(**resource)
    a = Action(**action)
    e = Environment(**env)
