...
export function FloatingAssistant() {
  ...
  return (
    <div className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] left-4 z-[80] print:hidden sm:bottom-6 sm:left-6" dir="rtl" aria-live="polite">
      {open && (
        <section
          className={[
          ...
          <header className="flex items-center justify-between border-b border-hsa-yellow/20 bg-hsa-yellow px-4 py-3 text-black">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full border border-black/20 bg-black shadow-inner">
                <img src={brand.assistant.iconPath} alt={brand.assistant.name} className="h-full w-full object-cover" />
              </div>
              <div>
                <p className="text-sm font-black">{brand.assistant.name}</p>
                <p className="text-xs font-semibold text-black/70">{brand.assistant.nameAr}</p>
              </div>
            </div>
          ...
          <div className="flex h-[calc(100%-4.25rem)] min-h-0 flex-col">
            <div className="border-b border-white/10 px-4 py-3">
              <div className="flex items-center gap-2 rounded-2xl border border-hsa-yellow/20 bg-hsa-yellow/10 px-3 py-2 text-xs text-slate-100">
                <ShieldCheck size={15} className="text-hsa-yellow" />
            ...
            <div ref={listRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain px-4 py-4">
              {messages.map((message, index) => (
                <div key={`${message.role}-${index}`} className={message.role === "user" ? "text-left" : "text-right"}>
                  ...
                  >
                    ...
                    {message.agent && (
                      <span className="mt-2 block text-[11px] font-bold text-hsa-yellow">Agent: {message.agent}</span>
                    )}
                  </div>
                </div>
              ))}
              {loading && (
                <div className="flex items-center gap-2 text-xs text-slate-300">
                  <Sparkles size={15} className="animate-pulse text-hsa-yellow" />
                  HSAAI يكتب الإجابة...
                </div>
              )}
            </div>
      ...
      <button
        onClick={openNewChat}
        className="group relative flex h-16 w-16 items-center justify-center sm:h-[72px] sm:w-[72px] rounded-full border-2 border-hsa-yellow bg-black text-hsa-yellow shadow-2xl shadow-hsa-yellow/20 transition hover:scale-105 hover:bg-hsa-yellow hover:text-black focus:outline-none focus:ring-4 focus:ring-hsa-yellow/30"
      ...
      >
        ...
        <img src={brand.assistant.iconPath} alt={brand.assistant.name} className="relative z-10 h-12 w-12 rounded-full object-cover sm:h-[58px] sm:w-[58px]" />
      </button>
    </div>
  );
}
