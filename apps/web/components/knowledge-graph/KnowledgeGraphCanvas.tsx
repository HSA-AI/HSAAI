  ...
  return (
    <div className="rounded-2xl border border-hsa-yellow/20 bg-white p-5 shadow-sm dark:bg-hsa-black">
      <div className="flex items-center justify-between gap-3">
        <p className="font-bold">Graph Overview</p>
      ...
      <div className="mt-5 grid min-h-80 gap-3 md:grid-cols-3">
        {visible.map((entity, index) => (
          <div key={entity.entity_key} className="rounded-2xl border border-hsa-yellow/20 bg-slate-50 p-4 dark:bg-hsa-black-soft" style={{ transform: `translateY(${(index % 3) * 8}px)` }}>
            <p className="text-xs font-bold text-hsa-gold">{entity.entity_type}</p>
            <p className="mt-1 font-bold">{entity.name}</p>
