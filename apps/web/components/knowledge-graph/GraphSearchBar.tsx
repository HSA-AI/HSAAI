export function GraphSearchBar({ value, onChange, onSearch }: { value: string; onChange: (value: string) => void; onSearch: () => void }) {
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-hsa-yellow/20 bg-white p-4 shadow-sm dark:bg-hsa-black md:flex-row">
      ...
      <button onClick={onSearch} className="rounded-xl bg-hsa-yellow px-5 py-3 font-bold text-hsa-black">بحث Graph</button>
    </div>
  );
}
