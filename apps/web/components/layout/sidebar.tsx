...
const visibleSections = enterpriseNavSections.filter((section) =>
  enterpriseNavItems.some((item) => item.section === section.key),
);
...
export function Sidebar({
  ...
  return (
    <aside
      className={`${
        mobile ? "flex h-dvh w-full" : "flex h-dvh w-80"
      } shrink-0 flex-col overflow-hidden border-l border-hsa-yellow/20 bg-white p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] dark:border-hsa-yellow/20 dark:bg-hsa-black`}
    >
      ...
      >
        <Image
          ...
          alt="HSAAI Enterprise Assistant"
          width={28}
          height={28}
      ...
      <nav
        ...
        aria-label="التنقل الرئيسي في HSAAI"
      >
                      ...
                      <span
                        className={`flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl ${
                          item.primary
                            ...
                            : "bg-slate-100 text-slate-700 dark:bg-white/5 dark:text-hsa-yellow"
                        }`}
                      >
                        {item.primary ? (
                          <Image
      ...
      <div className="mt-4 rounded-2xl border border-hsa-yellow/20 bg-hsa-soft/80 p-4 text-xs text-hsa-black dark:bg-slate-950 dark:text-slate-300">
        <p className="font-black text-hsa-black dark:text-hsa-yellow">
          Enterprise OS
        </p>

        <p className="mt-1 text-slate-600 dark:text-slate-400">
          Agent Mesh · RAG · Governance · FinOps · Integrations
        </p>
      </div>
    </aside>
  );
