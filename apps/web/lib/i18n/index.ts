...
i18n
  ...
  .init({
    ...
    // FIX (UI redesign): no `lng` override — when `lng` is set i18next skips
    // the detector AND the cache still writes the default on init, clobbering
    // the stored user preference on every reload. Detection order below gives:
    // stored preference → SSR html lang (ar, Arabic-first default).
