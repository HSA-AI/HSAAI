...
if db_url:
    ...
    # FIXED (audit P2-8): alembic.ini no longer carries a committed dev
    # password; fail with an actionable message instead of a cryptic parse
    # error deep inside SQLAlchemy.
    raise RuntimeError(
        "No database URL for Alembic. Set DATABASE_URL "
        "(e.g. postgresql+psycopg://user:pass@host:5432/hsaai) or set "
    ...
    for module_name in model_modules:
        try:
            ...
            if hasattr(mod, "Base") and hasattr(mod.Base, "metadata"):
                if not any(item is mod.Base.metadata for item in _all_metadata):
                    _all_metadata.append(mod.Base.metadata)
                print(f"  Registered metadata from {module_name}")
        except Exception as e:
            raise RuntimeError(f"Cannot load required migration metadata: {module_name}") from e
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Historic revision 0004 is longer than Alembic's default VARCHAR(32).
        # Preserve existing revision identities while supporting upgrades.
        if connection.dialect.name == "postgresql":
            from sqlalchemy import text
            connection.execute(text("SELECT pg_advisory_lock(481004)"))
        ...
        with context.begin_transaction():
            context.run_migrations()
        if connection.dialect.name == "postgresql":
            connection.execute(text("SELECT pg_advisory_unlock(481004)"))
            connection.commit()
