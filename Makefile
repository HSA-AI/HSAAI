...
help: ## Show this help message
	@echo "$(CYAN)HSAAI Enterprise AI Operating System$(RESET) — v3.0.0"
...
init-vault: ## Initialize Vault with policies and secrets
	# SECURITY FIX (audit P1-4): never cat approle.env — it printed the
	# VAULT_SECRET_ID to the terminal/logs. Show where to read it instead.
	@echo "Vault AppRole credentials are stored inside the container at"
	@echo "  /vault/config/approle.env (container: hsaai-vault-init)."
	@echo "Retrieve them out-of-band, e.g.:"
	@echo "  docker exec hsaai-vault-init sh -c 'grep -v SECRET_ID /vault/config/approle.env'"
	@echo "  # then fetch the secret_id over a secure channel (never logs)."
